...
- ```git commit -am "<(required) descriptive message of changes>"``` : adds and commits *previously tracked* files in one step (combines ```git add``` and ```git commit```)
