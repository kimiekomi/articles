...
One last topic...I almost forgot to mention one rather useful Git operation. There may be files within a repository that you do not want Git to track for one reasion or another. These files may be unnecessary to the project, contain sensitive data, generated by other processes, or simply not used by other collaborating developers. To avoid adding and commiting these individual files repeatedly, simply create a ```.gitignore``` file in your local repository and list the filenames you want Git to ignore. This procedure will hide all specified files from the ```git status``` and remain untracked. Untracked files that you may want Git to intentionally ignore include:
